...
I am showing the change of popularity of female names over the past decade (2008-2017)

```{r, eval= TRUE, echo=FALSE}
# subset dataset
summary <- babynames %>%
  filter(year %in% seq(2008, 2017, by = 1) & sex == "F") %>%
  group_by(year) %>%
  arrange(desc(n), .by_group = TRUE) %>%
  top_n(5, wt = n) %>%
  mutate(rank = row_number()) %>%
...
```{r, fig.align="center"}
summary %>%
  ggplot(aes(x = as.factor(year), y = rank, group = name))+
  geom_line(aes(color = name, alpha = 1), size = 2)+
  geom_point(aes(color = name, alpha = 1), size = 4)+
  geom_point(color = "#FFFFFF", size = 1)+
  geom_text(data = summary %>% filter(year == 2008), 
            aes(label = name), 
            fontface = "bold", color = "grey50", size = 3, hjust = 1.25)+
  geom_text(data  = summary %>% filter(year == 2017), 
            aes(label = name), 
            fontface = "bold", color = "grey50", size = 3, hjust = -0.45)+
  scale_y_reverse()+ # reverse your rank
  scale_x_discrete(expand = c(0.132, 0.132))+
  labs(title = "Popularity of Female Babynames Over the Past Decade (2008-2017)", 
       x = "Year", 
       y = "Ranking")+
  theme_bw()+
